...
with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
    ...
    while True:
        ...
        if server_message[:15] == "Your funds are:":  # Reading how much money it has
        ...
        elif "big blind bet is" in server_message:
            reverse_minbet = ""

            for char in server_message[::-1]:
                try:
                    int(char)

                    reverse_minbet = reverse_minbet + char

                except ValueError:
                    break

            minbet = int(reverse_minbet[::-1])
        ...
        elif server_message == "You do not have enough funds to raise":
            send("c")

        elif server_message[:38] == "You can't bet more money than you have":
            if server_message[40:-1] > funds:
                send("f")
            ...
            raiseTo = int(server_message[63:-1]) * 2
            ...
            rand_num = random.randint(1, hand_strength)  # Choosing a random percentage up until the strength of its hand

            bet = (rand_num / 10) * funds / 2  # That percentage of its funds / 2 will be its bet
