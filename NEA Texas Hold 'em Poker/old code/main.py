...
def lobby(funds, names):
    # connect game and players to netHost

    p1 = Player(funds, input(names[1]))

    p2 = Player(funds, input(names[2]))

    p3 = Player(funds, input(names[3]))

    p4 = Player(funds, input(names[4]))

    dealer = Player(0, "dler")
...
def opener(playerlist, funds):
    ...
    smallblind.smallblindbet(funds)
    ...
    print(smallblind.stats["Name"], "put down a small blind bet of", str(smallblind.stats["Bet"]), "\n"
          "This means the big blind bet is", str(bigblind.stats["Bet"]))
...
def main():
    ...
    playerlist, dealer = lobby(funds, names)
